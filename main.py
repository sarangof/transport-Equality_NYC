...
formula: monthly transportation cost / monthly income
we calculate the transporation cost for different modes (car, metro,.),
and multiply the cost of each mode to the corresponding proportion of
population that uses this mode as means of commute.
"""

#

Income2013 = pd.read_csv('data/2013Incomepc.csv',  header=1, names=[
                         "id", "id2block", "block", "PCincome", "error"], usecols=["id2block", "PCincome"])
Income2013.PCincome = Income2013.PCincome.replace("-", "0")
Income2013.PCincome = Income2013.PCincome.astype(int)
Mtrans2013 = pd.read_csv('data/Meanoftransportation.csv',
                         header=1, usecols=[1, 5, 21, 37])
...
Costcar2013 = pd.read_excel(
    'data/norteastCES2013.xlsx',  header=1, usecols=["Item", "New York"])
...
Costptrans2013['Pondcost'] = Costptrans2013.Porcentage * \
    Costptrans2013['Value per trip'] * Costptrans2013['Trips per year']
PTcost = sum(Costptrans2013.Pondcost)
Bcost = 95
CompA = pd.merge(Income2013, Mtrans2013, on=['id2block'])
CompA['CompA'] = np.log((CompA.Pcar * Carcost + CompA.PPTrans *
                         PTcost + CompA.PBike * Bcost) / (CompA.PCincome))
CompA[CompA.CompA > 1]
CompA = CompA[CompA.CompA < 1]
CompA['CompA1'] = (CompA.CompA - CompA.CompA.min()) / \
    (CompA.CompA.max() - CompA.CompA.min())
...
formula: commute time / commute distance
For this component we calculate the distance between centroids of census block
groups. Since the commute time estimate is given, we simple divide the time by
...
distW = pd.read_csv(
    'data/dist.csv', names=['ind', 'av_distance'], usecols=['av_distance'], header=0)
distW['av_distance'] = distW['av_distance'].astype(np.float64)
IT_index['CompB'] = timeW['av_time']


timeW = timeW.rename(columns={'Id2': 'id2block'})
IT_index = pd.merge(timeW[['id2block', 'av_time']], CompA[
                    ['id2block', 'CompA1']], on='id2block')
...
IT_index['CompB'] = (IT_index['CompB'] - IT_index['CompB'].min()) / \
    (IT_index['CompB'].max() - IT_index['CompB'].min())
...
author: Xia Wang
date: 03/08/2015
formula: normed(normed(No. Bus stops/CBG area) + normed(No. Subway stops/CBG area)
 + normed(bike lane length/CBG area))
"""
...
data_clean = data[['CBG_id', 'Num_Metro_update',
                   'Num_Bus_update', 'BlockGroupArea']]
...
# if one block group has more than one section of bike lane, add all the
# length to that CBG
...
# Bus_subindex
bike_merge['Bus_pre'] = bike_merge['Num_Bus_update'] * \
    1.0 / bike_merge['BlockGroupArea']
bike_merge['CompBus'] = ((bike_merge['Bus_pre'] * 1.0 -
                          bike_merge['Bus_pre'].min()) /
                         (bike_merge['Bus_pre'].max() -
                          bike_merge['Bus_pre'].min()))
# Metro_subindex
bike_merge['Metro_pre'] = bike_merge[
    'Num_Metro_update'] * 1.0 / bike_merge['BlockGroupArea']
bike_merge['CompMetro'] = ((bike_merge['Metro_pre'] * 1.0 -
                            bike_merge['Metro_pre'].min()) /
                           (bike_merge['Metro_pre'].max() -
                            bike_merge['Metro_pre'].min()))
# Bike_subindex
bike_merge['Bike_pre'] = bike_merge[
    'BikeLength'] / bike_merge['BlockGroupArea']
bike_merge['CompBike'] = ((bike_merge['Bike_pre'] * 1.0 -
                           bike_merge['Bike_pre'].min()) /
                          (bike_merge['Bike_pre'].max() -
                           bike_merge['Bike_pre'].min()))

# combine the 3 subindices and give them equal weight
bike_merge['CompC'] = 0.3333 * bike_merge['CompBus'] + 0.3333 * \
    bike_merge['CompMetro'] + 0.3333 * bike_merge['CompBike']
bike_merge['CompC_norm'] = ((bike_merge['CompC'] * 1.0 -
                             bike_merge['CompC'].min()) /
                            (bike_merge['CompC'].max() -
                             bike_merge['CompC'].min()))
...
#
IT_index['total'] = 1.0 / 3 * IT_index['CompA'] + 1.0 / \
    3 * IT_index['CompB'] + 1.0 / 3 * IT_index['CompC_norm']
